...
from overfero.utils.gcp_utils import wait_for_extended_operation
...
class InstanceTemplateCreator:
    ...
    def _machine_configuration(self) -> None:
    ...
    def _attach_metadata(self) -> None:
        ...
        for metadata_name, metadata_value in self.vm_metadata_config.items():
            self.template.properties.metadata.items.append(
                compute_v1.Items(key=metadata_name, value=str(metadata_value))
            )
